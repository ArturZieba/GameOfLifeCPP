...
//Game Of Life grid dimensions - const beacuse the std::array size is not dynamic
...
int main()
{
	...
	//Game Of Life grid arrays - using std::array<> instead of int[][] because of added functionality and instead of std::vector<> because these will not be resized at runtime and will not be big enough to warrant heap allocation
	...
	for (auto& element : gridPrinted)
	{
		...
		//Lambda has & in its definition so that generator/distribution are in its capture list
